...
const int nO[nZones] = {Lgd_Ord};
const int nE[nZones] = {nElem};

const double Leng[nZones] = {2.0};
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "iteration(f_iter) is %d\n", f_iter);
...
SEMdata *SEM2(int Ord)
{
    ...
    for (int i = 0; i < ndiv; ++i)
    {
        ...
        if (cnt)
        {
            ...
            fprintf(stderr, "the %2dth solution = %g. iteration = %d\n", idx, x, cnt);
        }
        else
        {
            fprintf(stderr, "Convergence not obtained \n");
        }
...
static void fun15(double *x, int *n, void *data, double *u, int *ok)
{
    ...
    *ok = 1;
    ...
    // print results
    FILE *xfptr = fopen("results\\u.csv", "w");
    FILE *yfptr = fopen("results\\ux.csv", "w");
    // FILE *dyfptr = fopen("results\\dy.csv", "w");
